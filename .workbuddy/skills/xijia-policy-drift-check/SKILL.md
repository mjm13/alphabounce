...
1. `rg "Gate-0|Gate-1|Gate-2|complete\\|partial\\|reject" .cursor/rules .cursor/skills .cursor/commands`
2. 检查状态模板、probe、Deferred、comment-sync、xijia-quality-judge 字段
3. 运行 `python .cursor/hooks/policy_flow_drift_check.py`
4. 修改 skill description 前：`python .cursor/skills/evals/scripts/run_skill_evals.py`
...
1. 逐条读 `.cursor/rules/*.mdc` 的 frontmatter `defends:`（缺失项由 `drift_guard_scan.py` 报出，见 `06-rule-drift-guard.mdc`）。
...
3. 删除候选先做**消融对照**：删前删后各跑 `run_skill_evals.py` + `pytest .cursor/hooks/tests -q`，无回归才落地。
