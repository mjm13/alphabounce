...
4. Table-First Panel 变更：`python .cursor/hooks/pipeline_guard.py --check-ui-pattern [--base HEAD]`
5. `python .cursor/hooks/pipeline_guard.py --check-comment-sync`
...
10. Gate-2 前：`python .cursor/hooks/pipeline_guard.py --check-release --req <file>`。若 Gate-2 前已有 blocking，应在提请 Gate-2 前尽量修复；若 Gate-2 签字后仍 blocking，**不得停轮**，在 Gate-3 链内修复后继续 Move。
