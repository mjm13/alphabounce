...
> Gate 初始仅写状态；人工确认通过后才追加 `审批人:<git user.name>；YYYY-MM-DD`（Gate-0 通过写 `已通过`）。按需片段见 `.cursor/templates/requirements/section-fragments.md`。
...
> UI 缺陷且无原型时必填（见 `.cursor/templates/requirements/gate1-plan-template.md` / `section-fragments.md`）；纯后端或样式单点可写 `不适用（无 UI 触达）`。
...
> hotfix 基于 `main` 修复时，合入后必须回合 `dev`。
