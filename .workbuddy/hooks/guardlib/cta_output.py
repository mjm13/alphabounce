...
def _step_instruction(ctx: CtaContext) -> str:
    ...
    if sub == "实现中":
        if ctx.release_blockers:
            return f"Agent：消除 release 阻塞 → `python .cursor/hooks/pipeline_guard.py --check-release --req {rel}`"
...
def _please_and_then(ctx: CtaContext) -> tuple[str, str]:
    ...
    rel = _req_rel_path(ctx.req_path)
    release_cmd = f"`python .cursor/hooks/pipeline_guard.py --check-release --req {rel}`"
    ...
    if sub == "Gate-0":
        return (
            ...
            f"Agent 回写 Gate-0 → `python .cursor/hooks/pipeline_guard.py --check-intake --req {rel}` → 再输出 CTA",
        )
    if sub == "Gate-1 方案缺口":
        return (
            "无（Agent 继续）",
            f"Agent 执行 A.0.5 + writing-plans 补 Gate-1 → `python .cursor/hooks/pipeline_guard.py --check-plan --req {rel}` → 再输出待批准 CTA",
        )
    if sub == "Gate-1 待批准":
