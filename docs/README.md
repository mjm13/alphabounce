...
本目录承载 **Alphabounce 安卓复刻** 的需求、方案、规格、决策与领域知识沉淀。

## 文档生命周期（必读）

| 类型 | 路径 | 行为 |
| --- | --- | --- |
| **活文档** | `AGENTS.md`、`docs/README.md`、`docs/llms.txt`、`docs/domain/`、`docs/decisions/`、`docs/patterns/`、`docs/pitfalls/` | Gate-3 **持续修正**；同一事实只保留一处 |
...
- `process/knowledge-maintenance.md`：活文档更新触发表
- `process/project-lifecycle.md`：生命周期阶段对照
- `process/release-checklist.md`：发布清单
- `process/incident-response.md`：事故响应
- `requirements/defect-template.md`：缺陷模板
...
7. 发布前维护 `process/release-checklist.md`；事故见 `process/incident-response.md`
...
- `architecture.md`：需求收尾阶段需要沉淀架构决策时创建/更新
- `capability-map.md`：业务需求 Gate-3 **动态合并**能力索引（`extract_capability_index.py`）；PRD Step 1.7 对照；`/xijia:overview` 速览
- `patterns/`、`pitfalls/`：经验文档（运行时创建，Gate-3 维护）
