...
class AnonSprite9160381 extends display.ASprite {
	public var field:pixi.core.text.Text;
	public var icon:display.ASprite;
}

class AnonSprite9160380 extends display.ASprite {
	public var scroll:AnonSprite9160381;
}

class Option extends Phys { // }
	public inline static var AIMANT = 0;
	public inline static var BLINDAGE = 1;
	public inline static var COLLE = 2;
	public inline static var DIMINUTION = 3;
	public inline static var EXTENSION = 4;
	public inline static var FLAMME = 5;
	public inline static var GLACE = 6;
	public inline static var HALO = 7;
	public inline static var INDIGESTION = 8;
	public inline static var JAVELOT = 9;
	public inline static var KAMIKAZE = 10;
	public inline static var LASER = 11;
	public inline static var MULTIBALL = 12;
	public inline static var NOUVELLE_BALLE = 13; // 	SWAP ( COLOR )
	public inline static var OUVRE = 14;
	public inline static var PROVISION = 15;
	public inline static var QUASAR = 16;
	public inline static var RALLENTISSEMENT = 17;
	public inline static var SAUVETAGE = 18; // 	SWAP
	public inline static var TRANSFORMEUR = 19;
	public inline static var ULTRA_VIOLET = 20;
	public inline static var VOLT = 21;
	public inline static var WHISKY = 22;
	public inline static var XANAX = 23;
	public inline static var YOYO = 24; // 	:-/
	public inline static var ZELE = 25;
	...
	public function new(mc) {
		...
		vy = FALL_SPEED;
		skin = cast root;
		skin.smc = skin.createEmptyMovieClip("smc", 0);
		skin.anchor.set(0.5, 0.5);
		skin.scroll = cast skin.createEmptyMovieClip("skin", 0);

		var mask = new Graphics();
		mask.beginFill(0xFF0000);
		mask.drawRect(-10, -15, 20, 10);
		skin.scroll.mask = mask;
		skin.scroll.position.set(-8, -5);
		skin.addChild(mask);

		skin.scroll.initTextField("field", {
			size: 16,
			x: 9,
			y: 8,
			align: "center",
	...
	public function setType(n) {
		...
		if (n < 26) {
			...
			skin.scroll.field.style.fill = Col.objToCol(o);
		} else {
			skin.scroll.gotoAndStop(2 + (n - 26));
			switch (n) {
				case 26: // MISSILE
					skin.scroll.smc = skin.scroll.attachMovie("mcMissile", "smc", 0);
					skin.scroll.smc.gotoAndStop(Game.me.missileType + 1);
			}
	...
	public function setItem(n) {
		...
		root.smc.blendMode = pixi.core.Pixi.BlendModes.ADD;
		for (i in 0...16)
			newRay();
		var icon = sdm.attach("mcMapIconSmc", 1);
	...
	function apply() {
		...
		for (i in 0...max) {
			var plu = Game.me.dm.attach("partLineUp", Game.DP_PARTS);
			plu.anchor.set(0.5, 0.5);
			var p = new fx.LineUp(plu);
		...
		if (flItem) {
			...
			var text = if (id < 18) {
				Text.get.ITEM_NAMES[id];
			} else if (id > 58) {
				Text.get.ITEM_NAMES[id - 39];
			} else '${Text.get.ITEM_NAMES[19]} ${id - 18}';
			Game.me.newTitle(text.toUpperCase(), 0x000044, null, 60);
		} else {
	...
	function newRay() {
		...
		sp.root.anchor.set(0, 0.5);
		sp.vr = (Math.random() * 2 - 1) * 1.5;
		// sp.fr = 0.95;
		sp.root.play();
		sp.root.stopOnFrame = [6];
