...
class Pod extends display.ASprite {
	public var p:Phys;
	public var sens:Int;
}
...
class Pad extends PadSkin { // }
	...
	public function new() {
		...
		landPodLength = 0;
		if (Cs.pi.shopItems[ShopInfo.PODS] == 1)
			landPodLength = 14;
		if (Cs.pi.shopItems[ShopInfo.PODS_EXTEND_0] == 1)
			landPodLength += 10;
		if (Cs.pi.shopItems[ShopInfo.PODS_EXTEND_1] == 1)
			landPodLength += 10;
		if (Cs.pi.shopItems[ShopInfo.PODS_EXTEND_2] == 1)
			landPodLength += 10;

		acc = 0.15;
		if (Cs.pi.shopItems[ShopInfo.LANDER_REACTOR_0] == 1)
			acc += 0.15;
		if (Cs.pi.shopItems[ShopInfo.LANDER_REACTOR_1] == 1)
			acc += 0.2;
		if (Cs.pi.shopItems[ShopInfo.LANDER_REACTOR_2] == 1)
			acc += 0.3;
	...
	function takeOff() {
		...
		weight = WEIGHT * lander.Game.me.currentPlanet.g;
		vx = 0;
		vy = -2 * lander.Game.me.currentPlanet.g;
		removePods();
	...
	function incFuel(inc) {
		...
		if (lander.Game.me.mcInter.fuel.smc != null) {
	...
	function initLandingControl() {
		...
		if (lander.Game.me.currentPlanet.type == 1 || Cs.pi.gotItem(MissionInfo.COMBINAISON)) {
	...
	function removePods() {
		while (pods != null && pods.length > 0)
			pods.pop().removeMovieClip();
	}
	...
	function showMinerai(n) {
		...
		var field:pixi.core.text.Text = (cast p.root).field;
		field.text = Std.string(n);

		p.x += (16 + field.width) * 0.5;
	}

	// TOOLS
	function getMouseAngle() {
		var dx = lander.Game.me.base._xmouse - Cs.mcw / 2;
		var dy = lander.Game.me.base._ymouse - Cs.mch / 2;
	...
	override public function explode(dm) {
		if (dm == null)
			dm = lander.Game.me.bdm.empty(lander.Game.DP_UNDERPARTS);
		super.explode(dm);
