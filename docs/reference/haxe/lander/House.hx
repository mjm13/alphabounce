...
class House { // }
	...
	public var mask:display.ASprite;
	public var mcAlien:display.ASprite;
	...
	public function new(rnd, scn) {
		...
		// POSITION
		var ma = 100;
		var mc = BmpTextureHelper.getASprite("mcHouse");
		mc.anchor.set(88 / 234, 0.5);
		...
		// Y
		var my = y - (lander.Game.HEIGHT - game.currentPlanet.dMax);
	...
	public function dig() {
		...
		var m = new Matrix();
		m.scale(sx, 1);
		m.translate(x, y - (lander.Game.HEIGHT - game.currentPlanet.gMax));
		// game.bmpGround.draw(mc, m, null, "erase"); // OUch
		trace('FIXME');
		mc.removeMovieClip();
	}
	...
	public function active() {
		...
		if (mask == null) {
			...
			mask.anchor.set(88 / 234, 0.5);
			mask._x = x;
			mask._y = y;
			mask.gotoAndStop(4);
			// mask.blendMode = pixi.core.Pixi.BlendModes.EXCLUSION; // fixme : ERASE
			mask._xscale = sx * 100;
		}
	}
	...
	static var SCN_STATS = [
		{scn: Casino, w: 0}, {scn: Swapper, w: 0}, {scn: Treasure, w: 6}, {scn: Traveler, w: 15}, {scn: Furi, w: 10}, {scn: Seller, w: 7}, {scn: Info, w: 8},
		{scn: Empty, w: 10}, {scn: Locked, w: 10}, {scn: Locked, w: 10},
	];
	...
	static public function getScenario(zid, seed:mt.OldRandom) {
		...
		for (i in 0...12)
			a.push(MissionInfo.TBL_0 + i);
		for (i in 0...42)
			a.push(MissionInfo.EMAP_0 + i);
		...
		if (lander.Game.me.flHouseVisited)
			return Empty;

		// DESERT
		if (lander.Game.me.currentPlanet.pop * seed.rand() < 1)
			return null;

		// INHABITE
		if (lander.Game.me.currentPlanet.pop == null)
			return null;
