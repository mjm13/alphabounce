...
class Mineral { // }
	...
	public function setValue(n) {
		...
		val = n;
		life = 30 + n * 10;
		root._xscale = root._yscale = 60 + n * (5 / (skinId * 7 + 1));

		root.gotoAndStop(skinId + 1);
	}
	...
	public function collect() {
		...
		var max = Std.int(Num.mm(3, val, 50));
		var ma = -0.5;
		for (i in 0...max) {
	...
	public function getPosModifier() {
		...
		while (true) {
			...
			var b = mc.getBounds();
			var x = (Math.random() - 0.5) * (b.right - b.left);
			var y = -Math.random() * (b.bottom - b.top) * 0.5;

			var px = x + root._x + mc._x;
			var py = y + root._y + mc._y;

			if (root.hitTest(px, py, true)) {
				return {x: x, y: y};
			}
