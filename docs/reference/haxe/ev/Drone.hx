...
class Drone extends Event { // }
	...
	public function new() {
		...
		if (Cs.pi.shopItems[ShopInfo.DRONE_PERFO] == 1)
			transformSpeed = 0.005;
		if (Cs.pi.shopItems[ShopInfo.DRONE_SPEED] == 1)
			jumpSpeed = 18;
	...
	public static function getBlock() {
		...
		if (bl == null && Cs.pi.shopItems[ShopInfo.DRONE_COLLECTOR] == 1)
			bl = getRandomBonusBlock();
		/*
	...
	public static function getRandomBonusBlock() {
		...
		for (x in 0...Cs.XMAX) {
			for (y in 0...Cs.YMAX) {
				if (Game.me.grid != null) {
					var bl = Game.me.grid.get('${x},${y}');
					if (bl != null && Block.isBonus(bl.type))
						list.push(bl);
				}
			}
		}
	...
	public static function getRandomSentiBlock() {
		...
		for (x in 0...Cs.XMAX) {
			for (y in 0...Cs.YMAX) {
				if (Game.me.grid != null) {
					var bl = Game.me.grid.get('${x},${y}');
					if (bl != null && Block.isSentinelle(bl.type))
						list.push(bl);
				}
			}
		}
	...
	function updateBlock() {
		...
		if (flConverter)
			become = Block.BONUS;
		if (bl.type != Block.LURE)
			bl.incTransform(transformSpeed * mt.Timer.tmod, become);
