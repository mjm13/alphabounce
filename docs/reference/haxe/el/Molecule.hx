...
class Molecule extends Element { // }
	...
	public function setType(n) {
		...
		for (n in mols) {
			...
			sp.root._xscale = sp.root._yscale = 25;
			newTrg(sp);
			list.push(sp);
		}
	}
	...
	override public function update() {
		...
		for (sp in list) {
			...
			if (Math.abs(dx) + Math.abs(dy) < 0.2 || Std.random(20) == 0)
				newTrg(sp);
		...
		root.filters = null;
		Filt.glow(root, Std.int(2 + 5 * c), 1 + 2 * c, 0xFFFFFF);
	}
	...
	override function onEnterSquare(sx:Null<Int>, sy:Null<Int>) {
		if (flDeath)
			return;
		// Game.me.monsterGrid.get('${px-sx},${py-sy}').remove(this);
		// Game.me.monsterGrid.get('${px},${py}').push(this);
	...
	override function onBounce(px, py) {
		switch (type) {
			...
			case ARMORER:
				...
				if (bl != null && bl.life < 5 && bl.type < 5) {
	...
	function insertInGrid(x, y) {
		for (dx in 0...3) {
			for (dy in 0...3) {
				var nx = x + dx - 1;
				var ny = y + dy - 1;
				if (Game.me.monsterGrid.get('${nx},${ny}') == null)
					Game.me.monsterGrid.set('${nx},${ny}', []);
				Game.me.monsterGrid.get('${nx},${ny}').push(this);
			}
		}
	}
	...
	function removeFromGrid(x, y) {
		for (dx in 0...3) {
			for (dy in 0...3) {
				...
				Game.me.monsterGrid.get('${nx},${ny}').remove(this);
			}
		}
	}
