...
class Ball extends Element { // }
	...
	function initFamily() {
		fam = 0;
		if (Cs.pi.gotItem(MissionInfo.BALL_DRILL))
			fam = 1;
		if (Cs.pi.gotItem(MissionInfo.BALL_SOLDAT))
			fam = 2;
		if (Cs.pi.gotItem(MissionInfo.BALL_POWER))
			fam = 3;
		if (Cs.pi.gotItem(MissionInfo.BALL_BLACK))
			fam = 4;
		#if test
		fam = 1;
		#end
	...
	override public function update() {
		...
		if (gluePoint != null) {
			...
			if (Game.me.flFirstBall)
				Game.me.flFirstBall = false;
		}
		...
		switch (type) {
			...
			case Cs.BALL_KAMIKAZE:
				...
				if (trg != null) {
					...
					if (Math.random() * 60 / mt.Timer.tmod < 1)
						trg = null;
				}
				Game.me.plasmaDraw(root);
	...
	public function colPad(cx:Float) {
		...
		if (flPierce) {
			pierceDamage = damage;
			if (type == Cs.BALL_STANDARD)
				root.smc.gotoAndStop(1 + fam);
		}
	}
		...
		if (type != n) {
			...
			switch (n) {
				case Cs.BALL_STANDARD:
					root.smc = root.attachMovie("ballMain", "smc", 0);
					root.smc.anchor.set(0.5, 0.5);
					root.smc.gotoAndStop(1 + fam);
				case Cs.BALL_FIRE:
					root.smc = root.attachMovie("mcFire", "smc", 0);
					root.smc.anchor.set(2/3, 0.5);
					root.smc.loop = true;
					root.smc.play();
				case Cs.BALL_VOLT:
					root.smc = root.attachMovie("mcVolt", "smc", 0);
					root.smc.anchor.set(0.5, 6/13);
					root.smc.loop = true;
					root.smc.play();
			}
		}
	...
	override function onBounce(px:Int, py:Int) {
		...
		if (flPierce) {
			if (pierceDamage > 0) {
				var bl = Game.me.grid.get('${px},${py}');
				if (bl != null && bl.life != null) {
					pierceDamage -= bl.life;
					if (pierceDamage > 0 && type != Cs.BALL_ICE)
						flBounce = false;
				}
			} else if (type == Cs.BALL_STANDARD) {
				root.smc.gotoAndStop(fam + 2);
			}
		}
	...
	override function onEnterSquare(sx, sy) {
		var monArray = Game.me.monsterGrid.get('${px},${py}');
		if (monArray != null && monArray[0] != null) {
			var mon = monArray[0];
			var mp = mon.getPos();
			var bp = getPos();
			var dx = bp.x - mp.x;
		...
		/*
			...
			for( dx in 0...3 ){
				for( dy in 0...3 ){
					var nx = px+dx-1;
					var ny = py+dy-1;
					for( mon in Game.me.monsterGrid.get('${nx},${ny}') )list.push(mon);
				}
			}
	...
	function genSparks(fr, turn) {
		if (Std.random(Sprite.spriteList.length) < 20) {
			for (i in 0...1) {
				...
				p.root.smc = p.root.attachMovie(if (fr == 1) 'sparkFire' else 'sparkIce', 'smc', 0);
				p.root.smc.anchor.set(0.5, 0.5);
				p.root.smc.play();
				p.root.smc.stopOnFrame = [22];
				p.x = x;
				p.y = y;
	...
	public function fxLight() {
		...
		for (i in 0...max) {
			...
			var ultra = dm.attach("mcUltraRay", 0);
			ultra.play();
			ultra.removeObjOnFrame = 15;
			ultra.anchor.set(0.5, 1);
			var sp = new Phys(ultra);
	...
	public function fxPowerUp() {
		var mc = new mt.DepthManager(root).attach("fxBallPowerUp", 0);
