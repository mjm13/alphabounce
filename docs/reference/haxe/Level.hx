...
class Level { // }
	...
	public function genPalette() {
		...
		var bmpPaint = RenderTexture.create(Cs.XMAX, Cs.YMAX);
		bmpPaint.fill(zone.col);
		var brush = BmpTextureHelper.getSprite("mcBrush");
		...
		for (i in 0...16) {
			...
			var col = Col.objToCol({r: r, g: g, b: b});
			brush.tint = col;
			brush.alpha = 0.4;

			bmpPaint.draw(brush, m);
		}

		this.bmpPaint = PixelHelper.extract(bmpPaint);
	}

	static public var DEBUG_FORCE_OPTION_TYPE = null; // DEBUG ONLY

	public function genBonusTable() {
		bonusTable = new StringMap();
		...
		for (x in 0...Cs.XMAX) {
			for (y in 0...Cs.YMAX) {
				if (seed.rand() < Cs.OPTION_COEF) {
					bonusTable.set('${x},${y}', Option.getRandomId(seed));
				}
				if (DEBUG_FORCE_OPTION_TYPE != null) {
					bonusTable.set('${x},${y}', DEBUG_FORCE_OPTION_TYPE);
				}
			}
		}
	}
	...
	public function genPrimeModel() {
		...
		var bmp = RenderTexture.create(Cs.XMAX, Cs.YMAX);

		// MASSE
		var brush = BmpTextureHelper.getASprite("mcShape");
		var sc = 0.06;
		var ma = -2;
		...
		for (i in 0...max) {
			...
			(cast Manager.app.renderer).render(brush, cast {renderTexture: bmp, clear: false, transform: m});
		}

		var pixels = new PixelHelper(untyped Manager.app.renderer.plugins.extract.pixels(bmp), Cs.XMAX, Cs.YMAX);
				...
				for (y in 0...ymax) {
					if (model.get('${x},${y}') < 5) {
						for (d in Cs.DIR) {
							var nx = x + d[0];
							var ny = y + d[1];
							if (nx >= 0 && nx < Cs.XMAX && ny >= 0 && ny < ymax + 1 && model.get('${nx},${ny}') == null) {
			...
			if (seed.random(proba[PB_DRAGON]) == 0) {
				...
				for (i in 0...max) {
					...
					if (model.get('${x},${y}') != null)
						model.set('${x},${y}', Block.DRAGON_LEFT + n);
				}
			}
		...
		if (flMirror) {
			...
			for (x in 0...mx) {
				...
				for (y in 0...ymax) {
					var n = model.get('${x},${y}');
					model.set('${nx},${y}', n);
					if (n == Block.DRAGON_LEFT)
						model.set('${nx},${y}', Block.DRAGON_RIGHT);
					if (n == Block.DRAGON_RIGHT)
						model.set('${nx},${y}', Block.DRAGON_LEFT);
				}
				// seemingly never worked to begin with
				//if (flMirrorPalette) PixelHelper.copyPixels(bmpPaint, bmpPaint, new pixi.core.math.shapes.Rectangle(x, 0, 1, Cs.YMAX), new pixi.core.math.Point(nx, 0));
			}
		}
	...
	function genZoneModif() {
		switch (zid) {
			...
			case ZoneInfo.DOURIV:
				...
				if (seed.random(4) == 0)
					genRandom(Block.INSECT, 3 + seed.random(12));
			...
			case ZoneInfo.BALMANCH:
				...
				for (i in 0...2) {
					...
					for (n in 0...max) {
						...
						var y = seed.random(ymax);
						model.set('${x},${y}', [Block.DRAGON_RIGHT, Block.DRAGON_LEFT][i]);
					}
				}
			...
			case ZoneInfo.FOLKET:
				...
				for (i in 1...6)
					searchAndReplace(i, Block.SPACE);
			...
			case ZoneInfo.EARTH:
				...
				/*
					...
					while(seed.random(max)==0){
						max++;
						var list = getHoriLine(1+seed.random(ymax-2),0.1, null, 2 );
						for( p in list )model.set('${p[0]},${p[1]}', Block.GUARDIAN);
					}
				 */
		}
	}
	...
	function getRandomBlock() {
		...
		while (true) {
			var x = seed.random(Cs.XMAX);
			var y = seed.random(ymax);
			var id = model.get('${x},${y}');
			if (id > 0 && id != Block.MINE)
				return {x: x, y: y};
			if (to++ > 100)
				break;
		}
	...
	public function getScreenshot(sc:Float, ?defaultColor:Int) {
		...
		var bmp = RenderTexture.create(ww, hh);
		var mc = navi.Map.me.dm.empty(0);
		...
		for (x in 0...Cs.XMAX) {
			for (y in 0...Cs.YMAX) {
				...
				if (type != null) {
					...
					bl.dm.clear(0);
					if (bl.type == 0 && defaultColor != null)
						bl.setColor([defaultColor]);
					/*var m = new Matrix();
						m.scale(sc, sc);
	...
	public function isModelOpen(obj) {
		...
		for (x in 0...Cs.XMAX) {
			if (sx == null && Block.isSoft(model.get('${x},${sy}')))
				sx = x;
		}
		if (sx == null) {
	...
	function getPath(x:Int, y:Int, grid:StringMap<Bool>, profondeur) {
		...
		for (d in Cs.DIR) {
			var nx = x + d[0];
			var ny = y + d[1];
			if (grid.get('${nx},${ny}') == null && nx >= 0 && nx < Cs.XMAX && ny >= 0 && ny < ymax + 1) {
				if (Block.isSoft(model.get('${x},${y}')))
