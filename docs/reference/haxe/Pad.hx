...
class Pad extends PadSkin { // }
	...
	public function new(mc) {
		...
		//
		var max = 1;
		if (Cs.pi.gotItem(MissionInfo.BALL_DOUBLE))
			max++;
		for (i in 0...max)
			initStartBall();
			...
			case Cs.PAD_TIME:
				if (flTransform && power > 0) {
					...
					if (trg != null) {
						...
						// FX
						var mc = new ASprite();
						var mmc = mc.createEmptyMovieClip();
						...
						for (i in 0...2) {
							...
							var mmmc = BmpTextureHelper.getASprite("mcTransformerImpact");
							mmmc.anchor.set(0.5, 0.5);
							mmmc._x = bx;
							mmmc._y = by;
							mmc.addChild(mmmc);
						}
	...
	public function bounceBall(ball:el.Ball) {
		if (!flInit) checkDrone(ball.x, ball.y);
	...
	function updateCharge() {
		...
		for (i in 0...2) {
			var line = Game.me.dm.attach("partLine", Game.DP_PARTS);
			line.anchor.set(1, 0.5);
			var p = new fx.Attract(line);
			var ec = 12;
	...
	function unSelectBlocks() {
		while (selectedBlocks != null && selectedBlocks.length > 0)
			selectedBlocks.pop().select(false);
		selectedBlocks = [];
		opx = null;
	}
	...
	function checkMissiles() {
		...
		if (Game.PLAY_AUTO && Game.me.balls.length < 3) {
			...
			flTest = KeyboardManager.isDown(32) || KeyboardManager.isDown(X_KEY);
		}

		if (flTest && Game.me.missile > 0) {
			if (missileCoef == null)
				missileCoef = 0;
	...
	function updateFlash() {
		...
		root.filters = null;
		if (c > 0)
			Filt.glow(root, Std.int(16 * c), 2 * c, 0xFFFFFF);
	}
