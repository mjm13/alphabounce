...
class Traveler extends navi.menu.People { // }
	...
	override function init() {
		...
		// MINERAI
		if (bs.random(10) > 0)
			rewardMin = Std.int(dist * (5 + bs.random(15)));

		// CAPS
		if (bs.random(3) == 0)
			rewardCaps = Std.int(dist * bs.rand() * 0.15);
	...
	public function accept() {
		...
		h.mark();
		mission._timestamp = Date.now().getTime();

		h.alienBehaviour = 0;
		lander.Game.me.travel = mission;
		if (rewardMin != null)
			lander.Game.me.incMinerai(rewardMin);
		if (rewardCaps != null)
			lander.Game.me.incCaps(rewardCaps);
	...
	public function getIntroText(info) {
		...
		str = Str.searchAndReplace(str, "::rmin::", Std.string(rewardMin));
		str = Str.searchAndReplace(str, "::rcap::", Std.string(rewardCaps));
