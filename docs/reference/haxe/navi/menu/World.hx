...
class World extends navi.Menu { // }
	...
	var pts:Array<Array<display.ASprite>>;
	...
	function initBg() {
		...
		var col = 0x002266;
		/*for (x in 0...xmax)
				bmp.fillRect(new flash.geom.Rectangle(Std.int(x * sq), 0, 1, Cs.mch), col);
			for (y in 0...ymax)
				bmp.fillRect(new flash.geom.Rectangle(0, Std.int(y * sq), Cs.mcw, 1), col); */
		trace("FIXME");
	}
	...
	function initIcons() {
		...
		for (a in ZoneInfo.holes) {
			for (i in 0...a.length) {
				...
				mc.gotoAndStop(4);
				if (i == 0)
					path.moveTo(mc._x, mc._y);
				if (i == 1)
					path.lineTo(mc._x, mc._y);
			}
		}
		...
		bmpZone.destroy();
		bmpZone = RenderTexture.create(Cs.mcw, Cs.mch);
