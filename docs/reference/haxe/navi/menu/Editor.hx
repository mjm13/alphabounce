...
class Brush extends display.ASprite {
	public var bl:Block;
	public var field:pixi.core.text.Text;
	public var n:Int;
}

class Vig extends display.ASprite {
	public var id:Int;
...
class Editor extends navi.Menu { // }
	...
	var grid:StringMap<Block>;

	var zone:display.ASprite;
	...
	function initBg() {
		// BG
		bg = dm.empty(0);
		bmpBg = RenderTexture.create(Cs.mcw, Cs.mch);
		bg.attachBitmap(bmpBg, 0);

		// LINES
		/*var col = 0x005500;
			for (x in 0...Cs.XMAX)
				bmpBg.fillRect(new flash.geom.Rectangle(Std.int(Cs.getX(x)), 0, 1, Cs.mch), col);
			for (y in 0...Cs.YMAX)
				bmpBg.fillRect(new flash.geom.Rectangle(0, Std.int(Cs.getY(y)), Cs.mcw, 1), col); */
		trace("FIXME");
	...
	function updatePaint() {
		var bl = grid.get('${mx},${my}');

		if (bl.type != brush.bl.type) {
			if (bl == null) {
				bl = new Block(mx, my, brush.bl.type, dm.empty(2), true);
				grid.set('${mx},${my}', bl);
			} else {
	...
	// BLOCK
	function removeBlock(x:Int, y:Int) {
		var bl = grid.get('${x},${y}');
		incBrush(brushes[bl.type], 1);
	...
	function newBrush(id) {
		if (brushTypes[id] != true) {
			if (!Cs.pi.flAdmin || !Block.isBasic(id))
				return;
		}
		...
		brush.bl = new Block(null, null, id, cast brush.smc, true);

		if (id == 0)
			brush.bl.setColor([DEFAULT_COLOR]);
		var max = 10;
		brush._x = Cs.getX(brushIndex % max);
		brush._y = Cs.getY(Cs.YMAX + Std.int(brushIndex / max) - 0.5);
		brushes[id] = brush;
		brush.onPress = select.bind(brush);
		brush.n = 0;
		if (Cs.pi.flAdmin)
			brush.n = 999;
		incBrush(brush, 0);
	...
	public function loadDefault() {
		var level = new Level(Cs.pi.x, Cs.pi.y, map.zoneTable.get('${Cs.pi.x - navi.Map.SX},${Cs.pi.y - navi.Map.SY}'), true);
		level.flEdit = true;
		level.genModel();
	...
	public function getString() {
		var tab = new StringMap();
		for (x in 0...Cs.XMAX) {
			for (y in 0...Cs.YMAX) {
				tab.set('${x},${y}', grid.get('${x},${y}').type);
			}
		}
	...
	public function loadString(str:String) {
		...
		if (a.length > 100) {
			...
			var tab = haxe.Unserializer.run(str);
			for (x in 0...Cs.XMAX) {
				for (y in 0...Cs.YMAX) {
					var n = tab.get('${x},${y}');
					if (n != null) {
						var bl = new Block(x, y, n, cast dm.attach("mcBlock", 2), true);
						if (bl.type == 0)
							bl.setColor([DEFAULT_COLOR]);
						grid.set('${x},${y}', bl);
					}
