...
from guardlib.hookio import (
    extract_paths as _extract_paths,
    read_hook_payload as _read_hook_payload,
)
...
def _run_hook() -> int:
    ...
    print(
        ...
        "运行 `python .codebuddy/hooks/pipeline_guard.py --audit` 核对阶段证据。"
    )
    return 0
...
def _release_content_issues(base: str, req: Path, text: str, changed_files: list[str]) -> ReleaseResult:
    ...
    if spec_gaps:
        ...
        result.messages.append(
            ...
            "`python .codebuddy/hooks/pipeline_guard.py --check-ui-pattern` exit 0。"
        )
    return result
    ...
    if gate1_ok and not gate2_ok:
        ...
        if _business_hybrid_needs_distill(text):
            ...
            result.messages.append(
                ...
                f"python .codebuddy/hooks/pipeline_guard.py --gate3-trigger-report --req {req_rel}"
            )
...
def _run_check_release_readiness() -> int:
    ...
    try:
        ...
        if branch and branch.strip() != "master":
            warn.append(f"当前分支为 {branch.strip()!r}，封版应从 master 切出 release/<版本>")
