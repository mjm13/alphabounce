...
def _adopt_required_issues() -> list[str]:
    hard: list[str] = []
    if not (paths.ROOT / ".codebuddy/rules/00-workflow.mdc").is_file():
        hard.append("H1: 缺少 .codebuddy/rules/00-workflow.mdc")
    if not (paths.ROOT / ".codebuddy/hooks/pipeline_guard.py").is_file():
        hard.append("H1: 缺少 .codebuddy/hooks/pipeline_guard.py")
    required = (
...
def _agents_and_policy_issues() -> AdoptResult:
    ...
    script = paths.ROOT / ".codebuddy/hooks/policy_flow_drift_check.py"
