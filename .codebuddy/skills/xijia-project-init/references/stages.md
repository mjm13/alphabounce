...
- MCP 配置（可选）：是否按路径生成 `.codebuddy/mcp.json`；是否需要只读 DB MCP（需用户提供连接信息，init 不预设凭据）
...
- `.codebuddy/templates/requirements/defect-template.md`（缺陷/hotfix 模板 SSOT）
...
- **MCP（按需）**：用户确认后从 `templates/.codebuddy/mcp.json.tmpl` 渲染；未确认则跳过
...
```bash
python .codebuddy/skills/xijia-project-init/scripts/render_init_templates.py \
...
```powershell
# inbox 复制骨架 SSOT 在 .codebuddy/templates/requirements/（init 不渲染到 docs/）
# 读：必须 -Encoding utf8
...
- 在 `docs/requirements/` 再维护 `*-template.md` 副本（SSOT 见 `.codebuddy/templates/requirements/`）
- 对公共区三份 inbox 骨架预填 `{{author}}`/`{{date}}`（须保留占位符供 `/xijia:prd` / `/xijia:defect` 复制）

**docs-render 后抽检**：确认 `.codebuddy/templates/requirements/` 三份骨架 frontmatter 首行应为 `标题:` 而非乱码；`docs/requirements/` 下不应出现 `*-template.md`。
...
5. 安装到 `.codebuddy/skills/`（必要时从 `.agents/skills/` 搬运）
...
4. `driftScan`
5. `policyFlowDrift`：`python .codebuddy/hooks/policy_flow_drift_check.py`
