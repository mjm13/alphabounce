...
6. 安装后必须校验 `SKILL.md` frontmatter：`name` 与目录同名；**项目自建技能**须 `xijia-` 前缀（见 `.codebuddy/rules/07-xijia-skill-naming.mdc`）
7. 若 `skills` CLI 安装到 `.agents/skills/`，必须搬运到 `.codebuddy/skills/`
