...
落盘缺陷 inbox 前**必须** Read 本文件。骨架：`.codebuddy/templates/requirements/defect-template.md`。
...
抽取：现象、复现与环境、期望、版本/trace/日志、是否 hotfix（基于已发布 `release/<版本>` 修 prod）。  
...
5. 填 Gate-0 复现与期望 / 范围 / 数据流（或无新增声明）及 Gate-1（无原型+UI 触达时 **`## 页面布局预览`** → `## 验收标准` + `## 实现方案`；规则见 `.codebuddy/templates/requirements/gate1-plan-template.md` / `section-fragments.md` § Gate-1 / 页面布局预览）；勿另建独立 H1 验收标准
...
hotfix：提醒合入 `release/<版本>` 后必须回合 `master`（`46-git-branching.mdc`）。
