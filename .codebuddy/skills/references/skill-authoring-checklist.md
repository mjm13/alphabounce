...
对照 Matt Pocock `writing-great-skills`（可预测性优先），约束本仓库 **自建** `xijia-*` 技能。命名硬约束见 [`.codebuddy/rules/07-xijia-skill-naming.mdc`](../../rules/07-xijia-skill-naming.mdc)。
...
- [ ] 改 description 前先改 [`evals/skill-routing.eval.yaml`](../evals/skill-routing.eval.yaml)，再跑：
  ```bash
  python .codebuddy/skills/evals/scripts/run_skill_evals.py
  ```
...
5. `python .codebuddy/skills/evals/scripts/run_skill_evals.py`
6. 若改流程交叉文件：`python .codebuddy/hooks/policy_flow_drift_check.py`
...
4. `python .codebuddy/skills/evals/scripts/audit_skill_metrics.py`：desc/lines/negation 不得无说明回退
...
1. `python .codebuddy/skills/evals/scripts/audit_skill_metrics.py`
...
6. 抽 2–3 条真实会话做消融对照：无技能基线 vs 有技能（writing-skills TDD）；删前删后各跑 `run_skill_evals.py` + `pytest .codebuddy/hooks/tests -q`，无回归才落地
