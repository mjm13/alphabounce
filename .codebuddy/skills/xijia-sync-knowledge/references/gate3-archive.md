...
```bash
python .codebuddy/skills/xijia-docs-score/scripts/score_docs.py --judge-doc <doc-path> --judge-session <需求stem> --judge-verdict useful|neutral|misleading --judge-reason "<reason>"
```
...
```bash
python .codebuddy/hooks/pipeline_guard.py --gate3-trigger-report --req <inbox-path>
# JSON：追加 --json
```
...
# 或
python .codebuddy/hooks/pipeline_guard.py --check-gate3-preflight --req <inbox-path>
```
...
5. `python .codebuddy/skills/xijia-safe-file-write/scripts/verify_utf8.py <shipped-path>`（脚本已跑且 exit 0 可跳过）
6. `python .codebuddy/hooks/pipeline_guard.py --check-closeout --req <shipped-path>`（`--req` 必须为 shipped）
7. `python .codebuddy/hooks/pipeline_guard.py --check-doc-anchors`
...
```bash
python .codebuddy/hooks/extract_capability_index.py --req docs/requirements/inbox/<file>.md --dry-run
...
```bash
python .codebuddy/hooks/pipeline_guard.py --check-closeout --req <shipped-requirement-path>
