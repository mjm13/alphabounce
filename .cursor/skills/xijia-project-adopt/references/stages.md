...
```bash
python .cursor/hooks/scan_workspace.py --preflight-codegraph [--dry-run] [--skip-codegraph]
```
...
4. 渲染 `.cursor/mcp.json`（`codegraph-<moduleKey>` per ready module）
...
```bash
python .cursor/hooks/scan_workspace.py --discover [--skip-codegraph]
python .cursor/hooks/scan_workspace.py --ddd-discovery
```
...
```bash
python .cursor/hooks/pipeline_guard.py --check-adopt-readiness
python .cursor/hooks/policy_flow_drift_check.py
python .cursor/hooks/pipeline_guard.py --check-doc-links
```
