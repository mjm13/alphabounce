...
| **写后验证** | `python .cursor/skills/xijia-safe-file-write/scripts/verify_utf8.py <path>` | 同左 |
...
python .cursor/skills/xijia-safe-file-write/scripts/write_utf8.py <path> < content.md
```
...
6. `python .cursor/skills/xijia-safe-file-write/scripts/verify_utf8.py <shipped-path>`
7. Experience Reuse 与约束引用对齐
8. 标记对照 gate3-markers（说明内无误用 `no-op` 子串）
9. `python .cursor/hooks/pipeline_guard.py --check-closeout --req <shipped-path>`
10. `python .cursor/hooks/pipeline_guard.py --check-doc-anchors`
