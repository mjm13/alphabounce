...
# {{requirement_title}}
> Gate 状态与审批只维护在 YAML properties；`待*` 态仅写状态，其它状态须写 `审批人:<git user.name>；YYYY-MM-DD`。触达 UI 时 **Gate-1 批准前不要写** frontmatter `UI验收证据`（可省略该键）；批准同轮再写入 `UI验收证据: 组件测试|Playwright|集成测试`（默认组件测试）。按需片段见 `.cursor/templates/requirements/section-fragments.md`。Gate-1 字段见 `.cursor/templates/requirements/gate1-plan-template.md`；PRD 分档见 `.cursor/templates/requirements/gate1-by-tier.md`。
